...
enum stat { tx_enable = 0x2, rx_enable = 0x4 };
...
void svc_entry(unsigned int spsr, unsigned int elr, unsigned esr) {
  if (esr >> 26 != 0b010101) {
    uart_hex(esr);
    uart_puts("is NOT an aarch64 svc, see "
              "https://developer.arm.com/documentation/ddi0601/2020-12/"
 ...
 */
void disable_tx() { *AUX_MU_IER_REG &= 0x1; }
