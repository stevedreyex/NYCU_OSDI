...
unsigned int *load_prog(void *prog) {
  ...
  while (!memcmp(buf, "070701", 6) &&
    ...
    if (!memcmp(buf + sizeof(cpio_t), (char *)prog, ns - 1)) {
      uart_puts("HIT, at ");
      // Only one user program, hard writing return address to 0x90000000 for
      // further more applications
...
void print_initramfs() { uart_hex(mem); }
